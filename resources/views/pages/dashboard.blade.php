...
<div class="container-fluid">
        ...
        <div class="col-lg-3 col-md-12 col-sm-12">
            <div class="card mt-3 mb-1">
                <div class="card-body">
                    ...
                    <select class="unit-list form-control form-control-lg">
                        <option value="ALL" {{ $training === "ALL" ? "selected" : ""}}>ALL</option>
                        <option value="kementerian" {{ $training === "kementerian" ? "selected" : ""}}>Kementerian</option>
                        <option value="badan" {{ $training === "badan" ? "selected" : ""}}>Badan</option>
                        <option value="lembaga" {{ $training === "lembaga" ? "selected" : ""}}>Lembaga</option>
                        <option value="sekretariat" {{ $training === "sekretariat" ? "selected" : ""}}>Sekretariat</option>
                        <option value="direktorat" {{ $training === "direktorat" ? "selected" : ""}}>Direktorat</option>
                        <option value="pemda" {{ $training === "pemda" ? "selected" : ""}}>Pemda</option>
                        <option value="mahkamah" {{ $training === "mahkamah" ? "selected" : ""}}>Mahkamah</option>
                    </select>
                </div>
            </div>
        </div>
    ...
    <div id="chart-evagara-container" class="row justify-content-center">
    ...
    <div id="chart-unit-filter" class="row justify-content-center">
        <div class="col-lg-12 col-md-12 col-sm-12">
            <div class="card">
                <div class="card-body">
                    <canvas id="chart-main-unit-list"></canvas>
                    <a href="javascript:void(0)" class="btn btn-primary chart-main-unit-list">Unduh Diagram</a>
                    <a href="javascript:void(0)" class="btn btn-primary data-main-unit-list">Lihat Data</a>
                </div>
            </div>
        </div>
    </div>

    <div id="chart-trx-filter">
...
var steps;
    ...
    initiateChart('chart-main-unit-list', 'horizontalBar', 'Peserta Berdasarkan Kementerian / Badan / Sekretariat / Unit Kerja', `<?php echo $mainUnitList; ?>`, steps);
...
x = JSON.parse(x);
var ctxEvagaraCont = document.getElementById('chart-evagara-container');
ctxEvagaraCont.style.display = 'none';

if(x != 0) {
    ctxEvagaraCont.style.display = 'block';
...
var ctxChartTrxFilter = document.getElementById('chart-trx-filter');
var ctxChartUnitFilter = document.getElementById('chart-unit-filter');

const ls = window.location.href.split("/").pop();

if(ls === 'ALL') {
    ctxChartUnitFilter.style.display = 'none';
    ctxChartTrxFilter.style.display = 'block';
} else if(ls === 'kementerian' || ls === 'badan' || ls === 'lembaga' || ls === 'sekretariat' || ls === 'direktorat' || ls === 'pemda' || ls === 'mahkamah') {
    ctxChartUnitFilter.style.display = 'block';
    ctxChartTrxFilter.style.display = 'none';
} else {
    ctxChartUnitFilter.style.display = 'none';
    ctxChartTrxFilter.style.display = 'block';
}
...
$('.unit-list').select2();
$('.unit-list').on('select2:select', function (e) {
    var data = e.params.data;
    var start = `<?php echo $start; ?>`;
    var end = `<?php echo $end; ?>`;
        window.location = '/dashboard/' + start + '/' + end + '/' + data.id;
